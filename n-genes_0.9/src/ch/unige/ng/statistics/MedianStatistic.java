...
import ch.unige.ng.tools.utils.Median;
 ...
 * @version 16 d?c. 2004
 */
public class MedianStatistic<T extends GlobalIndividual> implements Statistic<Double, T> {
    ...
    public Double evaluate(Population<T> population) {
        ...
        final double fitnesses [] = new double[size];
        for (int i = 0; i < size; i++)
            fitnesses[i] = population.get(i).getFitness();
        return Median.median(fitnesses);
    }
